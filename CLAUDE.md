...
- If in a new project, before doing any other research, dive into the project and familiarise yourself with it. Give a summary of the internal working and what the project does.
- Before making statements like 'x is a known bug' or 'y works for this approach', make sure to actually do research whether that's actually the case.
...
- One trait per line.
- For getters of models, use getAttributeValue for attributes and getRelationValue for relations
...
### Migrations
- Always write both `up` and `down` methods — `down` must fully reverse what `up` does
